from estadosfinais import token
...
def analisador(codigoFonte):
                ...
                if texto == True:  # Se for texto
                    ...
                    if buffer and ncoluna == len(linha):
                        if testeVariavel(buffer) == False:
                            ...
                            lista_tokens.append(buffer + "|" + token('q61') + "|" + str(nlinha) + "|" + str(coluna))  # Guarda na lista de tokens o lexima com o seu token sua linha e coluna
                            estado_atual = "q0"
                            buffer = ""
                    ...
                    if i != " ":  # Verifica se nao e quebra de linha ou espaco
                        ...
                        if ncoluna < len(linha):
                            if transicao(estado_atual, linha[ncoluna]) == "error":
                                if token(estado_atual):  # Verifica se o estado atual é um estado final
                                    lista_tokens.append(buffer + "|" + token(estado_atual) + "|" + str(nlinha) + "|" + str(coluna))  # Guarda na lista de tokens o lexima com o seu token sua linha e coluna
                                    estado_atual = "q0"
                                    buffer = ""
                                    ...
                                    lista_erros.append("ERRO1 {} Caracter não esperado linha: {} e coluna: {}".format(i, nlinha, coluna))
                                    estado_atual = "q0"
                                    buffer = ""
                        else:
                            if token(estado_atual):  # Verifica se o estado atual é um estado final
                            ...
                            if estado_atual == "error" and testeVariavel(buffer) == False:
                                lista_erros.append("ERRO2 {} Caracter não esperado linha: {} e coluna: {}".format(i, nlinha, coluna))

                    if buffer and ncoluna == len(linha):
                        if testeVariavel(buffer) == False:
                            lista_erros.append("ERRO3 {} Caracter não esperado linha: {} e coluna: {}".format(i, nlinha, coluna))
                        else:
                            lista_tokens.append(buffer + "|" + token('q61') + "|" + str(nlinha) + "|" + str(coluna))  # Guarda na lista de tokens o lexima com o seu token sua linha e coluna
                            estado_atual = "q0"
                            buffer = ""
    ...
    if estado_atual == "error":
        lista_erros.append("ERRO4 {} Caracter não esperado linha: {} e coluna: {}".format(i, nlinha, coluna))
