    ...
    def inicia_geracao(self, **asm_param):
        ...
        for line in  self.codigo_intermediario:
            if 'kint' in line:
                ...
                assembly_logger.debug('Step: {}\t\t Detectado aritmetica')
                main.append('')
                ...
                assembly_logger.debug('Step: {}\t\tDetectado eskreva. Creating code for: {}'.format(line, line.split("eskreva")[1].strip()))
                ...
                assembly_logger.debug('Step: {}\t\tDetectado leiak. Creating code for: {}'.format(line, line.split("leiak")[1].strip()))
                ...
                assembly_logger.debug('Step: {}\t\tDetectado sek. Creating statement...'.format(line))
                ...
                assembly_logger.debug('Step: {}\t\t enkuanto Statement --> Jumping JMP to {}\n'.format(line, line.split()[1].strip()))
                ...
                if 'sek' in line:
                    assembly_logger.debug('Step: {}\t\tDetected sek. Creating statement...'.format(line))
                    main.append('')
                    ...
                    if '<' in line:
                        assembly_logger.debug('SEK Statement\t\tJumping JL to {}\n'.format(line.split()[6].strip()))
